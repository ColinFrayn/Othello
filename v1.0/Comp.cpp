...
int g_BestMove = -1, g_ThinkingMove = -1, g_Depth = -1;
...
void Comp(void *Ignore) {
  ...
  do {
    ...
    g_Depth = depth;
    (void)Search(B,depth,0,-INFINITY,INFINITY,bLastPass);

    // Loop until target depth reached and we've run out of time
    // Don't loop if there's only one legal move, obviously! ;)
  } while (!bAbortFlag && Count(PossibleMoves) > 1 && (depth++ < MaxDepth || GetElapsedTime() < (SearchTime * 75)));
...
// Start the search
int Search(BoardType *B, int depth, int ply, int alpha, int beta, bool bLastPass) {
  ...
  if (Moves == EMPTY) {
    ...
    // Recurse
    else score = -Search(B,depth-1,ply+1,-beta,-alpha,true);
    // Return the score
  ...
  while (Moves) {
    ...
    if (depth <= 1) score = -Score(B);
    // Recurse
    else score = -Search(B,depth-1,ply+1,-beta,-alpha,false);
    // Undo the move
    *B = Backup;
    if (bAbortFlag) break;
