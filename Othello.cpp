...
extern int g_BestMove, g_ThinkingMove, g_Depth, g_Score;
  ...
  switch (message) {
    ...
    case WM_COMMAND:
      ...
      switch (wmId) {
        ...
        case IDM_EDIT_UNDO:
          if (iHistoryPos > 0) {
            // Retract the last move, and keep going as long as it was a pass
            do {
              iHistoryPos--;
              // Undo the move on the board
              UndoMove(&Board,History[iHistoryPos],Undo[iHistoryPos]);
            } while (History[iHistoryPos]==-1 && iHistoryPos>0);
            bGameOver = false;
...
void DisplayBoard(BoardType *B, int hx, int hy) {
  ...
  if (g_Depth > 0) {
    ...
    char strTemp[20];
    if (g_Score < -(WIN-100)) sprintf(strTemp,"%d ply [L%d]",g_Depth,g_Score+WIN);
    else if (g_Score > (WIN-100)) sprintf(strTemp,"%d ply [W%d]",g_Depth,WIN-g_Score);
    else sprintf(strTemp,"%d ply [%.0f]",g_Depth,(float)g_Score/(float)SCORE_SCALE);
...
void Graphics_PrintString(char strText[], float scalex, float scaley) {
  ...
  for (unsigned int loop=0;loop<strlen(strText);loop++) {
